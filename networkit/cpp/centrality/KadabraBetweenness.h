...
class StateFrame {
public:
	std::atomic<count> nPairs;
	count epoch = 0;
	std::vector<std::atomic<count>> apx;

	void init(const count size) {
		apx = std::vector<std::atomic<count>>(size);
		nPairs.store(0, std::memory_order_relaxed);
	}

	void reset(count newEpoch) {
		for (count i = 0; i < apx.size(); ++i) {
			apx[i].store(0, std::memory_order_release);
		}
		nPairs.store(0, std::memory_order_release);
		epoch = newEpoch;
	}
};
